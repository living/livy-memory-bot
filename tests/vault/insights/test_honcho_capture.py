...
class TestLessonPath:
    def test_pr17_path_hash(self):
        ...
        path = build_lesson_path("2026-04-18T21:31:06-03:00", 17, "PR #17 — Evo Wiki Research Phase 2")
        assert path.name.endswith("-0d10c1.md")

    def test_pr18_path_hash(self):
        """PR #18 source_ref sha256[:6] = cdc07d."""
        path = build_lesson_path("2026-04-18T23:02:42-03:00", 18, "PR #18 — Batch-first")
        assert path.name.endswith("-cdc07d.md")

    def test_pr24_path_hash(self):
        """PR #24 source_ref sha256[:6] = 26ca99."""
        path = build_lesson_path("2026-04-21T00:25:27Z", 24, "PR #24 — Enriched Claims Rollout")
    ...
    def test_path_date_based_on_merged_at_brt(self):
        ...
        path = build_lesson_path("2026-04-22T02:25:27Z", 24, "PR #24")
        assert path.name.startswith("2026-04-21-")

    def test_idempotent_same_input(self):
        """Same inputs always produce same path."""
        p1 = build_lesson_path("2026-04-21T00:25:27Z", 24, "PR #24 — Enriched Claims Rollout")
        p2 = build_lesson_path("2026-04-21T00:25:27Z", 24, "PR #24 — Enriched Claims Rollout")
        assert p1 == p2

    def test_different_pr_different_hash(self):
        """Different PR numbers produce different hashes."""
        p17 = build_lesson_path("2026-04-18T21:31:06Z", 17, "PR #17")
        p24 = build_lesson_path("2026-04-21T00:25:27Z", 24, "PR #24")
        assert p17 != p24
