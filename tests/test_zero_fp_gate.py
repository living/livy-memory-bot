...
# Path import mirrors repo layout (skills/memoria-consolidation)
GATE_FILE = Path(__file__).resolve().parents[1] / "skills" / "memoria-consolidation" / "gate.py"


def _load_gate_module():
    """Load production gate module from repo path (fails until implemented)."""
...
def _load_strict_promotion_gate():
    return _load_gate_module().strict_promotion_gate
...
class TestZeroFPGateSafety:
    ...
    def test_gate_requires_tier_classification(self):
        ...
        strict_promotion_gate = _load_strict_promotion_gate()
    ...
    def test_all_criteria_present_required_for_promotion(self):
        ...
        # Strengthened: verify expected promotion behavior value, not just key presence
        assert result.get("promoted") is True, (
            f"Perfect case with all 5 criteria must be promoted. Got result: {result}"
        )
    ...
    def test_r005_cases_never_promoted_without_all_criteria(self):
        ...
        for original_case in cases:
            # Copy case before modifying (avoid mutating fixture)
            case = {**original_case, "tier": "A", "fact_check_result": {"passed": True}}
